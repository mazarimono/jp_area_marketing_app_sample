...
st.set_page_config(layout="wide", page_title="長目　みちび")
...
def make_fig(
    ...
    if area_set:
        fig.add_scattermapbox(
            lat=[lat_fl], lon=[lon_fl], marker={"color": "blue", "size": 30}
        )
        poly_geo = _df3.loc[1, "geometry"]
        lons, lats = poly_geo.exterior.coords.xy
        lons = list(lons)
        lats = list(lats)
        fig.add_scattermapbox(mode="lines", lon=lons, lat=lats, line={"width": 5})
    return fig


def add_market_size(
    gdf: gpd.GeoDataFrame, change_crs: str, market_size: int
) -> gpd.GeoDataFrame:
    """
...
def market_data(
    _base_df: gpd.GeoDataFrame, _sel_df: gpd.GeoDataFrame
) -> gpd.GeoDataFrame:
    """
...
def iryo_market_data(
    _iryo_df: gpd.GeoDataFrame, _sel_df: gpd.GeoDataFrame
) -> gpd.GeoDataFrame:
...
with st.sidebar:
    ...
    if area_set:
        ...
        lat_fl = st.slider(
            label="緯度選択", min_value=34.89, max_value=35.2, step=0.01, value=35.0
        )
        market_size = st.slider(
            label="エリアサイズ（m）", min_value=500, max_value=5000, step=1, value=1000
        )
        ...
        if iryo:
            ...
            picked_iryo = (
                pd.DataFrame(Counter(iryo1).items(), columns=["name", "count"])
                .sort_values("count", ascending=False)
                .reset_index(drop=True)
            )
    st.divider()
    st.link_button("長目にお問い合わせする", "https://chomoku.info/contact")
...
if area_set:
    fig = make_fig(
        df, iryo_df, select_col, n_hex, iryo, area_set, sel_gdf, lon_fl, lat_fl
    )
else:
